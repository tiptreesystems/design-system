...
| Althea Submit Feedback | Reverted | Candidate border changed `1px` → `0px`, width changed `150.078125px` → `148.078125px`, and the neighboring Cancel position shifted `2px`. |

These failures were not hidden with app-specific width adjustments. The final
consumer branches retain their original implementations.
